...
def load_cleaned_data(csv_path):
    ...
    return df, pmids, texts, abstract, mesh_terms_list
...
if __name__ == "__main__":
    ...
    # 文本嵌入参数
    text_model_name = "all-MiniLM-L6-v2"  # 文本嵌入模型
    batch_size = 256                         # 批处理大小
    
    # MeSH术语嵌入参数
    mesh_model_name = "all-MiniLM-L6-v2"  # MeSH术语嵌入模型
    
    # 加载数据（只需要加载一次）
    df, pmids, texts, abstract, mesh_terms_list = load_cleaned_data(input_file)
    
    # 嵌入文本（只需要嵌入一次）
    texts_embeddings = embed_texts(texts, model_name=text_model_name, batch_size=batch_size)
    
    # 定义alpha和beta的值范围，从0.05到0.95，步长为0.05
    alpha_values = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
    beta_values = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
    # 生成所有组合
